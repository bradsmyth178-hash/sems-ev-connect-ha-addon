...
if(d.charger_connection==='sems'&&(!d.sems_username||(!d.sems_password&&!window.__cfgd)||!d.wallbox_serial)){show('#o3',false,'Enter the GoodWe account and charger serial number.');return}if(d.charger_connection==='modbus'&&!d.charger_host){show('#o3',false,'Enter the charger IP address.');return}const pl=String(d.control_pin||'').length;if(pl>0&&pl<4){show('#o3',false,'A PIN you choose needs at least 4 characters \u2014 or leave it blank and we will make one for you.');return}show('#o3',true,'Saving and connecting…');try{const r=await withPin(()=>request('/api/save',d));const pinNow=d.control_pin||(r&&r.generated_pin)||'';if(pinNow)sessionStorage.setItem('sunlands-control-pin',pinNow);if(r&&r.generated_pin){pinPending=true;showGeneratedPin(r.generated_pin)}loadOut=r&&r.generated_pin?null:'#o3';loadFails=0;loadMax=15;setTimeout(load,900)}catch(e){show('#o3',false,e.message)}}
/* Set while a generated PIN is on screen and not yet acknowledged. load()
   refuses to hide the wizard while it is true, because hiding the wizard is
   what took the PIN away. */
let pinPending=false;
function ackPin(){pinPending=false;load()}
function showGeneratedPin(p){
  ...
  const box=$('#o3');
  box.className='out show ok';
  box.innerHTML='<b>Your control PIN is '+p+'</b><br>Write it down now \u2014 it is not shown again, and you need it to change these settings or use the controls on this page. It is already saved on this device, so charging works straight away.'+'<br><button type="button" class="btn" onclick="ackPin()" style="margin-top:10px">I have written it down</button>';
}
