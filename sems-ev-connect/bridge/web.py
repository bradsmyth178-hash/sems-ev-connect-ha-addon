...
<label class="mode"><input type="radio" name="charger_connection" value="sems" checked onchange="modeChanged()"><b>GoodWe Cloud</b><small>Use the GoodWe account that already shows this charger.</small></label>
...
<label class="field">GoodWe account email<input name="sems_username" type="email" autocomplete="username" placeholder="name@example.com"></label>
<label class="field">GoodWe account password<input name="sems_password" type="password" autocomplete="current-password" placeholder="Enter account password"></label>
<label class="field wide">Charger serial number<input name="wallbox_serial" autocomplete="off" placeholder="Serial number from the charger label or SEMS app"></label>
...
<div class="grid" style="margin-top:12px"><label class="field">Rated charger power<select name="charger_kw"><option value="7">7 kW</option><option value="11">11 kW</option><option value="22">22 kW</option></select></label>
<label class="field">Supply phases<select name="phases"><option value="1">Single phase</option><option value="3">Three phase</option></select></label></div>
<div class="actions"><button type="button" class="secondary" onclick="testCharger()">Test charger connection</button></div><div id="o1" class="out"></div></div>
...
<label class="field">Charger update interval (seconds)<input name="poll_seconds" value="30" type="number" min="5" max="120"></label><label class="field ocppOnly">OCPP meter interval (seconds)<input name="meter_seconds" value="30" type="number" min="5" max="3600"></label>
<label class="field wide">Local control PIN<input name="control_pin" type="password" minlength="4" placeholder="Choose at least 4 characters"><span>Protects start, stop, power and mode changes on your LAN. Never returned by the API. Leave blank when changing settings to keep the current PIN.</span></label>
<label class="field wide">Confirm the control PIN<input name="control_pin_confirm" type="password" autocomplete="off" placeholder="Type the same PIN again"><span>There is no way to recover this PIN later, so it is worth getting right now.</span></label>
<label class="field wide">SEMS EV CONNECT pairing code<input name="cloud_pairing" autocomplete="off" placeholder="Paste the pairing code from your SEMS EV CONNECT page"><span>Links this device to SEMS EV CONNECT so the charger can be watched and controlled from your customer page. Outbound-only — opens no ports.</span></label>
...
<p class="hint" id="pinWarn">Write the control PIN down before you save. It is the only way back into these settings, and it is never shown again.</p><div id="o3" class="out"></div></div></form></section>
...
async function findChargers(){
  const d=form();
  if(!d.sems_username||(!d.sems_password&&!window.__cfgd)){show('#oFind',false,'Enter the GoodWe account email and password first.');return}
...
def build_app(cfg: C.Config, state, restart_cb, control_cb) -> web.Application:
    ...
    async def find_chargers(req: web.Request):
        ...
        if not temp.sems_username or not temp.sems_password:
            raise web.HTTPBadRequest(text="enter the GoodWe account email and password first")
        link = SemsLink(
    ...
    async def save(req: web.Request):
        ...
        if pairing and not C.apply_pairing_code(draft, pairing):
            return web.json_response({"error": "that SEMS EV CONNECT pairing code was not recognised"}, status=400)
